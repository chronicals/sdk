...
export interface InternalConfig {
  apiKey: string
  endpoint: string
  routes?: ChronicalsRouteDefinitions
  routesDirectory?: string
  // TODO: Mark as deprecated soon, remove soon afterward
  actions?: Record<string, ChronicalsActionDefinition>
  // TODO: Mark as deprecated soon, remove soon afterward
  ...
  onError?: ChronicalsErrorHandler
...
export function getActionStore(): ChronicalsActionStore {
...
export function getPageStore(): ChronicalsPageStore {
  if (!pageLocalStorage) {
    throw new ChronicalsError(
      'Global io and ctx objects are only available in a Node.js context.'
    )
...
export function getSomeStore(): ChronicalsActionStore | ChronicalsPageStore {
  try {
    return getPageStore()
  } catch (err) {
...
export default class Chronicals {
  config: InternalConfig
  #logger: Logger
  #client: ChronicalsClient | undefined
  ...
  /**
   * Establish the persistent connection to Chronicals.
   */
  async listen() {
    if (!this.#client) {
      this.#client = new ChronicalsClient(this, this.config)
    }
  ...
  /**
   * Immediately terminate the connection to chronicals, terminating any actions currently in progress.
   */
  immediatelyClose() {
  ...
  /**
   * Safely close the connection to Chronicals, preventing new actions from being launched and closing the persistent connection afterward. Resolves when the connection is successfully safely closed.
   */
  ...
  /**
   * Sends a custom notification to Chronicals users via email or Slack. To send Slack notifications, you'll need to connect your Slack workspace to the Interval app in your organization settings.
   *
   * **Usage:**
   *
   * ```typescript
  ...
  async notify(config: NotifyConfig): Promise<void> {
    if (
      ...
      this.#log.warn(
        'Calls to notify() outside of a transaction currently have no effect when Chronicals is instantiated with a development API key. Please use a live key to send notifications.'
      )
    }
    ...
    try {
      ...
      throw new ChronicalsError('Invalid input.')
    }
  ...
  /**
   * Dequeue a previously assigned action which was created with `chronicals.enqueue()`.
   */

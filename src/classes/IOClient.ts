...
interface ClientConfig {
  ...
  // onAddInlineAction: (handler: ChronicalsActionHandler) => string
}
...
export class IOClient {
  ...
  // addInlineAction(handler: ChronicalsActionHandler): string {
   ...
   * responses (returns, state updates, or cancellations) from Chronicals.
   * Resolves when it receives final responses or from Chronicals,
      ...
      async (resolve, reject) => {
        ...
        const inputGroupResponseHandler: ResponseHandlerFn = async result => {
          try {
            ...
            // Transaction canceled from Chronicals cloud UI
  ...
  get io() {
    return {
      ...
      display: {
         ...
         *
         * Grid item size can be controlled using the idealColumnWidth property. Chronicals will calculate a column width that is as close as possible to that number while factoring in gutter size and window width.
         *

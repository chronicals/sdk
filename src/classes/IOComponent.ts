 ...
 * the data transmitted to Chronicals for an IO component, and handling responses
 * received from Chronicals.
 */
export default class IOComponent<MethodName extends T_IO_METHOD_NAMES> {
   ...
   * to determine the valid types for communication with Chronicals.
   * @param options.label - The UI label to be displayed to the action runner.
   * @param options.initialProps - The properties send to Chronicals for the initial
   * render call.
   * @param options.handleStateChange - A handler that converts new state received
   * from Chronicals into a new set of props.
  ...
  constructor({
    ...
    try {
      ...
      console.error(
        `[Chronicals] Invalid props found for IO call with label "${label}":`
      )
      console.error(err)
      throw err
    }

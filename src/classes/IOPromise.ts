 ...
 * the value received from Chronicals to a custom component return type.
 *
  ...
  multiple({
    ...
    if (defaultValue && 'defaultValue' in propsSchema.shape) {
      ...
      try {
        ...
        console.error(
          `[Chronicals] Invalid default value found for multiple IO call with label "${this.label}": ${defaultValue}. This default value will be ignored.`
        )
        console.error(err)

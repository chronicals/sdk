...
export type ActionCtx = {
  ...
  /**
   * Logs anything from your action by printing a message in the Chronicals dashboard. Works with multiple arguments like JavaScript’s console.log. Logs are truncated at 10,000 characters.
   *
   * **Usage:**
   *
   * ```typescript
  ...
  log: ActionLogFn
  /**
   * Sends a custom notification to Chronicals users via email or Slack. To send Slack notifications, you'll need to connect your Slack workspace to the Chronicals app in your organization settings.
   *
   * **Usage:**
   *
   * ```typescript
...
export type ChronicalsActionHandler = (
  io: IO,
  ctx: ActionCtx
) => Promise<IOFunctionReturnType | void>

export interface ChronicalsActionStore {
  io: IO
  ctx: ActionCtx
}

export interface ChronicalsPageStore {
  display: IO['display']
  ctx: PageCtx
}

export interface ExplicitChronicalsActionDefinition {
...
export type ChronicalsActionDefinition =
  | ChronicalsActionHandler
  | ExplicitChronicalsActionDefinition
  | Action

export type ChronicalsRouteDefinitions = Record<
  string,
  ChronicalsActionDefinition | Page
>

export type ChronicalsPageHandler = (
  display: IO['display'],
  ctx: PageCtx
...
export type ChronicalsErrorProps = {
  error: Error | unknown
  route: string
...
export type ChronicalsErrorHandler = (props: ChronicalsErrorProps) => void

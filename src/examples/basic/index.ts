...
const actionLinks: ChronicalsActionHandler = async () => {
  await io.group([
  ...
  routes: {
    ...
    html: async () => {
      ...
      await io.display.html('Restricted', {
        ...
        <h2>Heading 2</h2>
          ...
          <iframe src="https://chronicals.com"></iframe>
    ...
    section_heading: async io => {
      await io.group([
        io.display.heading('Section heading', {
          ...
          menuItems: [
            { label: 'Link', url: 'https://chronicals.com', theme: 'primary' },
    ...
    readonly_inputs: async io => {
      ...
      while (i < 2) {
        const responses = await io.group([
          ...
          io.input.email('Email input', { defaultValue: 'hi@chronicals.com' }),
          io.input
    ...
    confirmBeforeDelete: async (io, ctx) => {
      const email = await io.input.email('Enter an email address', {
        defaultValue: 'hello@chronicals.com',
      })
    ...
    richText: async io => {
      const [body, to] = await io.group([
        io.input.richText('Enter email body', {
          defaultValue:
            '<h2>Welcome to Chronicals!</h2><p>Enjoy your stay.</p>',
...
- one
  - two
    ...
    a_readonly_demo: async io => {
      await io.group(
        [
          io.input.text('Full name', { defaultValue: 'Chronicals' }),
          io.input.email('Email address', {
            defaultValue: 'hello@chronicals.com',
          }),
      ...
      routes: {
        ...
        single: async io => {
          const file = await io.input.file('Upload an image!', {
            helpText:
              'Will be uploaded to Chronicals and expire after the action finishes running.',
            allowedExtensions: ['.gif', '.jpg', '.jpeg', '.png'],
          })
    ...
    badMessage: async () => {
      const client = new ChronicalsClient(chronicals, chronicals.config)
